...
import { completeWorkOrder, deleteOrder, getIncompleteWorkOrders, updateWorkOrder } from "../../managers/workOrderManager";
...
export default function WorkOrderList({ loggedInUser }) {
    ...
    const completedWorkOrder = (workOrderId) => {
          ...
          {workOrders.map((wo) => (
            <tr key={wo.id}>
              ...
              <td>
                ...
                {wo.userProfile && (
                    <Button
                    onClick={() => completedWorkOrder(wo.id)}
                    color="success"
                    >
                    Mark as Complete
                    </Button>
